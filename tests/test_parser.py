...
def test_filepath():
    with pytest.raises(argparse.ArgumentTypeError):
        parser.filepath('move_along_no_file_to_see_here')

    assert __file__ == parser.filepath(__file__)


def test_output_path():
    for ext in io_utils.valid_ext:
        output_file = ''.join(['file', ext])
        assert output_file == parser.output_path(output_file)

    with pytest.raises(argparse.ArgumentTypeError):
        parser.output_path('file_with_bad_extension.666')
...
def test_get_parser(left_img, right_img):
    ...
    cmd = 'compose {left_img} {right_img}'.format(
        left_img=left_img['path'],
        right_img=right_img['path']
    )
    main_parser.parse_args(cmd.split())
